...
class ActionGoodsInfoWithMetaData(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        infoBoxGoods = tracker.get_slot('session_started_metadata')
        print(infoBoxGoods)
...
class ActionSlotSetterCode(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        code = tracker.get_slot('codes')  # todo change
...
class ActionHuman(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        dispatcher.utter_message(json_message={'text': "我已经记录了您的相关信息，现在我將為您轉人工服務！", 'types': 1})
